...
const crudOprtaions = {
  ...
  addProduct() {
    if (inputsValidation.emptyProductNameValidation()) {
      if (inputsValidation.productNameValidation()) {
        let product = {
    ...
    if (pList.length > 0) {
      ...
      for (let i = 0; i < pList.length; i++) {
        productsContainer += `<div class="col-md-4 ">
                  ...
                  <p>product categogry: ${searchTerm
                    ? pList[i].category
                        .toLowerCase()
                        .replace(
                          searchTerm,
  ...
  searchCategory() {
    //search by Category (phone-electronics-tv-tablets)
    const categoryList = [];
    let searchCategoryTerm = elements.searchCategory.value;
    for (let i = 0; i < productList.length; i++) {
      if (
        productList[i].category.toLowerCase().includes(searchCategoryTerm .toLowerCase())
      ) {
        categoryList.push(productList[i]);
      } else {
      }

      crudOprtaions.displayProducts(categoryList, searchCategoryTerm);
    }
  },
};
